...
class ReqView(APIView):
    def __init__(self, **kwargs):
        ...
        self._serverInfo = MyServer.restHandlersHelpers.readServerInfo("/app/serverInfo.log")
    ...
    def _getReqs(self, request):
        ...
        repoFolder, _ = MyServer.repoHelpers.getRepoInfo(self._serverInfo["usersFolder"], request)
        reqs = MyServer.restHandlersHelpers.getDocReqs(request.GET.get("docId"), repoFolder)
        if not reqs:
...
class DocView(APIView):
    ...
    def _addDocument(self, request):
        ...
        if not MyServer.restHandlersHelpers.addUserDocument(request.data.get("docId"), request.data.get("parentId"), repoFolder):
            return Response(
                {
                    "message": "Unable to add document. Could not build documents tree or root document exists and you need to specify the parent document or root document does not exist and you must not specify parentId."
                },
    ...
    def _getDocuments(self, request):
        ...
        repoFolder, _ = MyServer.repoHelpers.getRepoInfo(self._serverInfo["usersFolder"], request)
        serialized = MyServer.restHandlersHelpers.serializeDocuments(repoFolder)
        return JsonResponse(serialized, safe=False)
